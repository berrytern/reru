...
def run_benchmarks():
    ...
    for noise, pad in zip(noise, padding):
        ...
        print(f"{'Pattern':<25} | {'re (sec)':<10} | {'reru (sec)':<10} | {'Speedup'} | {'Selected Engine'}")
        print("-" * 65)
        ...
        for pattern, target in benchmarks:
            ...
            print(f"{display_pat:<25} | {re_time:<10.4f} | {reru_time:<10.4f} | {speedup:.2f}x | {reru.compile(pattern).engine_info()}")
